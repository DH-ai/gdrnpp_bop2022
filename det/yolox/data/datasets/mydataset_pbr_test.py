...
class MY_DATASET_PBR_TEST_Dataset:
    """MY Dataset bop test."""
...
SPLITS_MY_DATASET = dict(
    mydataset_pbr_test=dict(
        name="mydataset_pbr_test",
        dataset_root=osp.join(DATASETS_ROOT, "bop/test"),
        models_root=osp.join(DATASETS_ROOT, "bop/models"),
        # objs=MY_DATASET_BOP19_20_OBJS,  # selected 16 objects
        objs=ref.mydataset.objects,  # all 33 objects
        ann_file=osp.join(DATASETS_ROOT, "bop/test_targets.json"),
    ...
    # mydataset_pbr_test_primesense_all=dict(
    #     name="mydataset_pbr_test_primesense_all",
    #     dataset_root=osp.join(DATASETS_ROOT, "BOP_DATASETS/mydataset/test_primesense"),
    #     models_root=osp.join(DATASETS_ROOT, "BOP_DATASETS/mydataset/models"),
    #     objs=ref.mydataset.objects,  # all 33 objects
    #     ann_file=osp.join(DATASETS_ROOT, "BOP_DATASETS/mydataset/test_targets_all.json"),
    #     scale_to_meter=0.001,
    #     with_masks=True,  # (load masks but may not use it)
    #     with_depth=True,  # (load depth path here, but may not use it)
    #     height=1200,
...
for obj in ref.mydataset.objects:
    name = "mydataset_pbr_{}_test".format(obj)
    select_objs = [obj]
    if name not in SPLITS_MY_DATASET:
        SPLITS_MY_DATASET[name] = dict(
            name=name,
            dataset_root=osp.join(DATASETS_ROOT, "bop/test_primesense"),
            models_root=osp.join(DATASETS_ROOT, "bop/models"),
...
def register_with_name_cfg(name, data_cfg=None):
    ...
    if name in SPLITS_MY_DATASET:
        used_cfg = SPLITS_MY_DATASET[name]
    else:
...
def get_available_datasets():
    return list(SPLITS_MY_DATASET.keys())
...
if __name__ == "__main__":
    ...
    Usage:
        python -m core.gdrn_modeling.datasets.mydataset_pbr_test dataset_name
    """
